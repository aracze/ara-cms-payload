...
// šířky (okraj podle podkladu „logo-fb-small“, rozhodnutí 5. 9. 2026); při této velikosti
// sedí na středu a do kružnice se vejde. layout() navíc hlídá, aby rovné pravé rohy ležely
// uvnitř kružnice o poloměru ~48,5 % hrany — u většího ICON_FILL (zhruba od 68 %) papouška
...
if (!Number.isFinite(FILL) || FILL <= 0 || FILL > 1) {
  console.error(`ICON_FILL musí být číslo v rozsahu (0, 1], dostal jsem: ${process.env.ICON_FILL}`)
  process.exit(1)
}
...
/** Umístění papouška: svisle na střed; vodorovně na střed, dokud se rohy vejdou do kružnice, jinak doleva. */
...
/**
 * Vykreslí čtvercovou ikonu dané hrany (px) s papouškem podle layout(); `center` ho dá
 * vodorovně na střed bez ohledu na kružnici (maskable ikona), `background` je barva pozadí
 * nebo průhledno.
 */
...
/** Zapíše soubor relativně k výstupní složce (ICON_OUT, výchozí kořen repa). */
